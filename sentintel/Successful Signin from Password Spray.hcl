resource "my_alert_rule" "rule_307" {
  name = "Successful Signin from Password Spray"
  ...
  query = <<EOF
SecurityAlert
| where DisplayName == "Password Spray"
...
| distinct Time, Location, IPAddress, UserPrincipalName, UserDisplayName
EOF
  entity_mapping {
    entity_type = Account
    field_mappings {
  ...
  techniques = ['T1110']
  display_name = Successful Signin from Password Spray
  description = <<EOT
Correlates Password Spray with successful logins
EOT
  enabled = True
