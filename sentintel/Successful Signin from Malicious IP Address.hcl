resource "my_alert_rule" "rule_22" {
  name = "Successful Signin from Malicious IP Address"
  ...
  query = <<EOF
SecurityAlert
| where DisplayName == "Malicious IP address"
| extend IPAddress = tostring(parse_json(ExtendedProperties)["Client IP Address"]), Location = tostring(parse_json(ExtendedProperties)["Client Location"]),Id = tostring(parse_json(ExtendedProperties)["Request Id"])
...
| distinct Time, IPAddress, UserPrincipalName, UserDisplayName, AuthenticationRequirement, AppDisplayName, ClientAppUsed
EOF
  entity_mapping {
    entity_type = Account
    field_mappings {
  ...
  techniques = ['T1189']
  display_name = Successful Signin from Malicious IP Address
  description = <<EOT
This rule parses Malicious IP Address alerts and correlates successful signins.  
EOT
  enabled = True
