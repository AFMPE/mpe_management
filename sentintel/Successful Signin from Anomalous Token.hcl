resource "my_alert_rule" "rule_341" {
  name = "Successful Signin from Anomalous Token"
  ...
  query = <<EOF
SecurityAlert
| where DisplayName == "Anomalous Token"
| extend
...
| distinct Time, Location, IPAddress, UserPrincipalName, UserDisplayName, AuthenticationRequirement, AppDisplayName, ClientAppUsed
...
EOF
  ...
  techniques = ['T1078']
  display_name = Successful Signin from Anomalous Token
  description = <<EOT
Corralates Anomalous Token alerts with successful logins
EOT
  enabled = True
